# Batch Processing Tutorial: Searching for Ads using audfprint

This document describes how to use audfprint2 to locate instances of known TV ads (or other exact sound clips) within an archive of recorded TV shows (or other longer recordings).

It assumes we are running on a Linux or macOS host, with the relevant data files already accessible, in this example under the directory `/2/data/`.
